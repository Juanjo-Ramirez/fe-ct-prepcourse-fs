...
function numberOfCharacters(string) {
  ...
  const stringOdernado = string.toLowerCase().split('').sort()
  for (let i = 0; i < stringOdernado.length; i++){
    const caracter = stringOdernado[i]
    if (nuevoObjeto[caracter]){
      nuevoObjeto[caracter]++
    } else {
      nuevoObjeto[caracter] = 1
    }
  }
  return nuevoObjeto
}
